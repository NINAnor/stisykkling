...
#### DIRECTORIES AND NAMES - LANGSUA
  ...
  TrailObs.outname<-"Langsua_trailObs_EGNET"
  Trail.outname<-"Langsua_trails_EGNET"
...
#### DIRECTORIES AND NAMES - SJUNKHATTEN
  ...
  TrailObs.outname<-"Sjunkhatten_trailObs_EGNET"
  Trail.outname<-"Sjunkhatten_trails_EGNET"
...
print(paste("Trail",j,"of",IDUL))
  ...
  Sub<-Trail[tf,]
    ...
    C$x.min5<-c(rep(NA,5),C$x[1:(CL-5)])
    C$x.plus5<-c(C$x[6:CL],rep(NA,5))
    C$y.min5<-c(rep(NA,5),C$y[1:(CL-5)])
    C$y.plus5<-c(C$y[6:CL],rep(NA,5))
    C$Dist<-sqrt( (C$y.plus5-C$y.min5)^2 + (C$x.plus5-C$x.min5)^2)
    C$DEMchange<-c(C$DEM.plus5-C$DEM.min5)
    C$SlopePer<-abs(C$DEMchange*100/C$Dist)
    tf<-!is.na(C$Vert); C<-C[tf,]
    if(sum(!is.na(C$SlopePer))>0)
      {
      C$Vert<-C$DEM.min5<-C$DEM.plus5<-C$x.min5<-C$x.plus5<-C$y.min5<-C$y.plus5<-C$Dist<-C$DEMchange<-NULL
      C$path_ID<-IDU[j]
      CL<-nrow(C)
      C$point_ID<-seq(1,CL)
      C.ls[[j]]<-C
